...
def get_routes(requset, form) -> dict:
    context = {'form': form}
    ...
    travels_time = data['travels_time']
    cities = data['cities']
    ...
    if cities:
        ...
        if not right_ways:
            raise ValueError('Route through these cities impossible')
    ...
    for route in right_ways:
        ...
        if total_time <= travels_time:
            routes.append(tmp)
    if not routes:
        raise ValueError('Travel time is longer then possible')
    sorted_routes = []
    if sorted_routes == 1:
