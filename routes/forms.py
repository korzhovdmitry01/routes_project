...
class RouteForm(forms.Form):
    ...
    cities = forms.ModelMultipleChoiceField(
        label='Through cities', queryset=City.objects.all(), required=False,
        widget=forms.SelectMultiple(attrs={'class': 'form-control js-example-basic-multiple'}))

    travels_time = forms.IntegerField(label='Travel time', widget=forms.NumberInput(attrs={
        'class': 'form-control',
        'placeholder': 'Travel time'}))


class RouteModelForm(forms.ModelForm):
    name = forms.CharField(label='Name of route', widget=forms.TextInput(attrs={
        'class': 'form-control', 'placeholder': 'Enter a name of route'}))
